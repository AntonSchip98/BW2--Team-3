      ...
      cardBrano.addEventListener("click", function () {
        if (brano.title == brano.album.title) {
          ...
          getCallArtistAlbum("artist", brano.artist.id).then((artista) => {
            ...
            let targetBranoPopolare = artistPage.querySelector("#target-branoPopolare")
            ...
            fetch(`https://striveschool-api.herokuapp.com/api/deezer/artist/${id}/top?limit=50`)
              ...
              .then(tracks => {
                tracks.data.forEach((track, indice) => {
                  ...
                  let trackDuration = branoPopolare.querySelector(".tarckDuration")
                  let trackImgBranoPopolare = branoPopolare.querySelector(".trackImgBranoPopolare")
                  ...
                  trackDuration.innerHTML = convertiSecondiConPuntini(track.duration)
          ...
          getCallArtistAlbum("album", brano.album.id).then((album) => {
            ...
            let artistImgAlbumPage = albumPage.querySelector(".artistImgAlbumPage")
            ...
            function didascaliaAlbum() {
              didascaliaAlbumP.innerHTML = album.contributors[0].name + " ⋅ " + album.release_date.slice(0, 4) + " ⋅ " + album.nb_tracks + " brani, " + convertiSecondiConScritte(album.duration) + "."
            }
            // INSERISCO IL TEMPLATE DEL CLONE DELLA PAGINA ALBUM
            targetHome.append(albumPage)
            
            let targetAlbumPageBrano = document.querySelector("#target-templateBranoAlbum")
            album.tracks.data.forEach((track, indice) => {
              ...
              let titleAlbumTrack = albumPageBrano.querySelector(".titleAlbumTrack")
              console.log(titleAlbumTrack);
              let nameArtistAlbumTrack = albumPageBrano.querySelector(".nameArtistAlbumTrack")
              let rankAlbumTrack = albumPageBrano.querySelector(".rankAlbumTrack")
              let durationAlbumTrack = albumPageBrano.querySelector(".durationAlbumTrack")
              ...
              durationAlbumTrack.innerHTML = convertiSecondiConPuntini(track.duration)
